...
claims.json — список заявлений к подаче:
  [{"folder": "<имя папки в ~/Downloads/chaika>",
    ...
    "files":  ["Протокол приёма.pdf", "Акт оказанных услуг.pdf", "Кассовый чек.pdf"]  # опц.; если нет — автоподбор
  }]
Скан паспорта/полиса (profile.passportScanPath) прикладывается ко всем автоматически.
"""
...
def base_payload(prof, c):
    info = {"insuredName": prof["insured"]["insuredName"], "birthdate": prof["insured"]["birthdate"],
            "policyNumber": prof["policy"]["number"], "country": prof["insured"].get("country", "РФ"),
...
def files_payload(prof, c):
    ...
    files = [{"name": fn, "body": b64file(os.path.join(fdir, fn))} for fn in c["_files"]]
    scan = os.path.expanduser(prof["passportScanPath"])
...
def resolve_files(prof, c):
    ...
    miss = [fn for fn in want if not os.path.exists(os.path.join(fdir, fn))]
    scan = os.path.expanduser(prof["passportScanPath"])
...
def record_marker(prof, folder, no, c, status_msg):
    ...
    txt = (f"Заявление № {no}\n"
           f"Полис: {prof['policy']['number']} (ДМС, АльфаСтрахование)\n"
           f"Получатель: {prof['insured']['insuredName']}\n"
           ...
           f"Дата подачи: {today}\n"
           f"Статус: {status_msg}\n"
           f"Документы: {', '.join(c['_files'] + ['скан паспорта/полиса'])}\n")
...
def main():
    ...
    for c in claims:
        ...
        print("  файлы:", c["_files"] + ["<скан паспорта>"])
